 ...
 * return -1;
 */
int search_helper(int *arr, int key, int low, int high)
{
	int mid=0;
	if(low > high)
	{
	...
	{
		high = mid - 1;
		return search_helper(arr, key, low, high);
	}
	else
	{
		low = mid + 1;
...
{
	return search_helper(arr, key, 0, length-1);
